...
export const articles: HelpArticle[] = [
  ...
  },
  {
    ...
    "content": [
      ...
      {
        "heading": "Audio",
        "body": "Audio 탭은 파일을 업로드하고 버전을 관리하며 오디오를 재생하는 곳입니다. 헤더에는 작은 브레드크럼(아티스트 · 릴리스)이 표시되고, 그 아래 트랙 제목과 함께 이전 또는 다음 트랙으로 이동하는 좌우 셰브론 화살표가 있습니다. 화살표를 클릭해도 현재 탭은 유지됩니다(트랙 1의 Audio 탭에서 오른쪽 화살표를 클릭하면 트랙 2의 Audio 탭으로 이동합니다). 앨범을 탭 단위로 순서대로 탐색할 수 있습니다. 버전 선택기(v1, v2, v3 등)로 리비전 간 전환이 가능하며, + 버튼을 클릭하여 새 버전을 업로드합니다. 각 버전에는 버전 번호, 업로드 날짜, 댓글 수, 다운로드 버튼이 표시됩니다. 파형은 인터랙티브 재생을 지원합니다: 아무 곳이나 클릭하여 탐색하고, 아래 전송 컨트롤(루프, 뒤로 건너뛰기, 재생/일시정지, 앞으로 건너뛰기, 반복)을 사용합니다. 파형 바로 위에는 워커가 측정한 오디오 통계를 보여주는 QC 필이 표시됩니다: LUFS, True Peak(dBTP), 조건부 Quality 경고. 각 필을 클릭하면 자세한 설명이 나타납니다. 워커가 새로 업로드된 파일을 분석하는 동안에는 필이 표시될 위치에 \"측정 처리 중\"이라는 작은 표시가 나타납니다. 파형 아래 Feedback 섹션에서는 현재 버전의 모든 타임스탬프 댓글을 볼 수 있습니다. 파형을 더블클릭하면 해당 타임코드에 댓글을 추가할 수 있습니다.",
        "mockup": "track-tab-audio"
      },
      {
        "heading": "라우드니스 분석",
        "body": "오디오를 업로드하면 Mix Architect가 서버에서 자동으로 LUFS(Loudness Units Full Scale) 통합 라우드니스를 측정하고 캐시합니다. 버전 메타데이터 옆의 LUFS 필을 클릭하면 라우드니스 분석 팝오버가 확장됩니다. 이 팝오버는 주요 스트리밍 서비스, 방송 표준, 소셜 플랫폼이 재생 중 트랙을 어떻게 조정하는지 보여줍니다. 각 행에는 플랫폼 이름, 목표 라우드니스(예: Spotify는 -14 LUFS를 목표), 파일에 적용되는 게인 변화가 표시됩니다. 양수 값은 서비스가 트랙 볼륨을 높인다는 의미이고, 음수 값(주황색으로 표시)은 볼륨을 낮춘다는 의미입니다. 예를 들어, 믹스가 -14.9 LUFS로 측정되면 Spotify는 +0.9 dB를 적용하고 Apple Music(목표 -16)은 -1.1 dB를 적용합니다. 플랫폼 이름에 마우스를 올리면 해당 플랫폼이 오디오를 정규화하는 방식에 대한 간단한 설명을 볼 수 있습니다. 팝오버는 Streaming(Spotify, Apple Music, YouTube, Tidal, Amazon Music, Deezer, Qobuz, Pandora), Broadcast(EBU R128, ATSC A/85, ITU-R BS.1770), Social(Instagram/Reels, TikTok, Facebook)로 구분됩니다. 팝오버 외부를 클릭하면 닫힙니다. 이를 사용하여 납품 전에 마스터가 어떤 플랫폼에서 크게 변경되는지 확인할 수 있습니다.",
        "mockup": "track-tab-lufs"
      },
      {
        "heading": "True Peak",
        "body": "True Peak(dBTP)는 ITU-R BS.1770-4에 따라 4배 오버샘플링을 사용하여 샘플 간 피크 값을 측정합니다. 이는 원시 샘플 피크와 다릅니다. 손실 코덱(MP3, AAC, Ogg Vorbis, Opus)은 인코딩 중 샘플 간 오버슈트를 발생시킬 수 있어, 기본 샘플이 0 dBFS에 도달하지 않아도 들릴 정도의 클리핑을 일으킵니다. True Peak 필을 클릭하면 측정된 true peak가 각 플랫폼의 상한선과 어떻게 비교되는지 볼 수 있습니다. LUFS(목표에 맞추고 싶은 값)와 달리 true peak는 상한선입니다. 목표 이하로 유지하는 것은 항상 괜찮습니다. 각 행은 플랫폼, 상한선(대부분 -1 dBTP; Spotify Loud 모드와 Amazon Music은 -2 dBTP), 그리고 \"X.X dB 여유\"(녹색, 상한선 아래) 또는 \"+X.X dB 초과\"(주황 또는 빨강, 상한선 위)를 표시합니다. 메인 행의 dBTP 수치 옆 작은 배지는 같은 색상 규칙을 따릅니다: -1 dBTP 미만은 녹색, -1에서 0 사이는 주황, 0 초과는 빨강(DSP 체인을 클립시키는 샘플 간 오버). 플랫폼 이름에 마우스를 올리면 해당 상한선이 선택된 이유를 볼 수 있습니다.",
        "mockup": "track-tab-truepeak"
      },
      {
        "heading": "품질 검사",
        "body": "품질 검사 필은 조건부입니다. 워커가 업로드에서 플래그할 가치가 있는 것을 감지한 경우에만 표시됩니다. 깨끗한 믹스에는 필이 표시되지 않습니다. 표시될 때, 가벼운 단일 문제는 호박색, 여러 문제 또는 심각한 문제는 빨강입니다. 오늘날 세 가지 문제 유형이 표시됩니다: 클리핑(풀 스케일 샘플 수가 많고 샘플 피크가 0 dBFS에 매우 가까움, 리미터 상한선에 걸터앉은 전형적 징후), 풀 스케일 샘플 피크(가장 큰 샘플이 ≥ -0.1 dBFS로, 하류 DSP 또는 손실 인코딩용 여유가 없음), DC 오프셋(0.002를 초과하는 0이 아닌 평균 진폭, 일반적으로 게인 스테이지 또는 필터링 문제). 필을 클릭하면 감지된 각 문제, 간단한 기술적 설명, 실행 가능한 수정(예: 클리핑에는 \"출력 게인을 낮추거나 리미터 상한선을 확인하세요\", DC 오프셋에는 \"20 Hz 이하의 하이패스 필터를 적용하세요\")이 있는 팝오버가 확장됩니다.",
  ...
  {
    "id": "studio-workspace",
    "title": "Studio 탭: 팀, 브랜딩, 도메인",
    "category": "studio",
    "summary": "Studio 탭에서 팀, 포털 브랜딩, 브랜디드 클라이언트 이메일, 커스텀 포털 도메인을 관리합니다.",
    "tags": [
      "studio",
      "팀",
      "워크스페이스",
    ...
    "content": [
      {
        "heading": "Studio 탭에는 무엇이 있나요",
        "body": "사이드바의 [Studio 탭](/app/studio)은 Studio 플랜의 워크스페이스 기능을 한곳에 모아 보여줍니다: 팀 멤버, 포털 브랜딩, 브랜디드 이메일, 커스텀 포털 도메인. 같은 카드가 [설정](/app/settings)에도 표시되므로 두 곳 어디에서든 관리할 수 있습니다. Free 및 Pro 플랜에서는 대부분의 카드에 기능 설명과 함께 업그레이드 안내가 표시됩니다. 포털 브랜딩은 예외로, Pro 플랜에도 포함되어 있습니다."
      },
      {
        "heading": "팀 멤버와 공유 프로젝트",
        "body": "이메일로 팀원을 워크스페이스에 초대하고 각자에게 역할을 지정합니다. 워크스페이스의 모든 구성원은 전체 카탈로그를 공유합니다: 릴리스별 초대 없이도 누구나 스튜디오 그룹의 모든 릴리스를 열고 작업할 수 있습니다. 역할은 수행할 수 있는 작업을 결정합니다. 관리자(Admin)와 엔지니어(Engineer)는 릴리스를 편집하고, 오디오를 업로드하고, 브리프를 관리할 수 있습니다. 뷰어(Viewer)는 읽기 전용으로 접근합니다. 릴리스 삭제는 워크스페이스 소유자와 관리자만 할 수 있습니다. 초대받은 팀원이 초대된 이메일 주소로 로그인하면 초대가 자동으로 수락되며 공유 릴리스가 대시보드에 표시됩니다.",
        "tip": "아무것도 변경하지 않고 진행 상황만 지켜봐야 하는 클라이언트나 이해관계자에게는 뷰어 역할을 사용하세요."
      },
      {
        "heading": "포털 브랜딩",
        "body": "로고를 업로드하고 강조 색상을 선택하면 공유하는 모든 클라이언트 포털에 스튜디오의 아이덴티티가 적용됩니다. 클라이언트가 다크 모드에서 포털을 볼 때 표시되는 다크 모드용 로고를 선택적으로 추가할 수 있습니다. SVG 업로드(또는 2~3배 크기로 내보낸 PNG)는 고해상도 화면에서도 선명하게 유지됩니다. Studio 플랜에서는 포털이 완전한 화이트 라벨로 제공됩니다: \"Powered by Mix Architect\" 푸터가 제거됩니다. 포털 브랜딩은 Pro와 Studio 모두에서 사용할 수 있습니다."
      },
      {
        "heading": "브랜디드 클라이언트 이메일",
        "body": "Studio 플랜에서는 클라이언트에게 보내는 이메일(포털 초대, 견적, 업데이트)이 스튜디오 이름으로 발송되며 로고와 강조 색상이 적용됩니다. Reply-To 주소를 설정하면 클라이언트의 답장이 내 받은 편지함으로 도착합니다. 비워 두면 답장은 계정 이메일로 전송됩니다."
      },
      {
        "heading": "커스텀 포털 도메인",
        "body": "mixarchitect.com 대신 portal.yourstudio.com 같은 자체 도메인에서 클라이언트 포털을 제공할 수 있습니다. 커스텀 포털 도메인 카드에서 도메인을 추가한 다음, 표시되는 DNS 레코드(서브도메인은 CNAME, 루트 도메인은 A 레코드, 필요한 경우 확인용 레코드 포함)를 사용하여 도메인을 Vercel로 연결합니다. 레코드가 전파될 때까지 카드에 Pending DNS 상태가 표시되며, 이후 Verified로 바뀌고 SSL이 포함된 새 도메인에서 포털이 자동으로 제공됩니다.",
        "tip": "portal.yourstudio.com 같은 서브도메인이 가장 쉬운 설정입니다. CNAME 레코드 하나면 끝납니다."
      }
    ]
  }
];
