...
export const articles: HelpArticle[] = [
  ...
  },
  {
    ...
    "content": [
      ...
      {
        "heading": "音频",
        "body": "音频选项卡用于上传文件、管理版本和播放音频。标题显示一个小型面包屑（艺术家 · 发行），其下是曲目标题和左右箭头，点击可前往发行中的上一首或下一首曲目。点击箭头会保留当前选项卡（从曲目 1 的音频选项卡跳转到曲目 2 的音频选项卡），让您可以按选项卡逐一浏览整张专辑。版本选择器（v1、v2、v3 等）可切换不同修订版；点击 + 按钮上传新版本。每个版本显示版本号、上传日期、评论数量和下载按钮。波形支持交互式播放：点击任意位置跳转，使用下方的传输控件（循环、后退、播放/暂停、前进、重复）。波形上方的一排 QC 胶囊显示工作器测量的音频统计数据：LUFS、True Peak (dBTP) 以及条件性的质量警告。点击任意胶囊可查看完整说明。工作器仍在分析新上传文件时，会在胶囊位置显示一个小「正在处理测量」指示器。波形下方的反馈部分显示当前版本的所有时间戳评论。在波形上双击可在该时间码添加评论。",
        "mockup": "track-tab-audio"
      },
      {
        "heading": "响度分析",
        "body": "上传音频时，Mix Architect 会在服务器端自动测量 LUFS（满刻度响度单位）积分响度并缓存。点击版本元数据旁的 LUFS 胶囊可展开响度分析弹出框。弹出框显示各主要流媒体服务、广播标准和社交平台在播放时将如何调整您的曲目。每行显示平台名称、其目标响度（例如 Spotify 目标为 -14 LUFS）以及将应用于您文件的增益变化。正值表示服务会调高您的曲目；负值（以橙色显示）表示会调低。例如，如果您的混音测量值为 -14.9 LUFS，Spotify 会应用 +0.9 dB，而 Apple Music（目标 -16）会应用 -1.1 dB。将鼠标悬停在平台名称上可查看该平台如何正规化音频的简短说明。弹出框分组为流媒体（Spotify、Apple Music、YouTube、Tidal、Amazon Music、Deezer、Qobuz、Pandora）、广播（EBU R128、ATSC A/85、ITU-R BS.1770）和社交（Instagram/Reels、TikTok、Facebook）。点击弹出框外部可关闭。使用此功能检查您的母带在交付前是否会在任何平台上被显著调整。",
        "mockup": "track-tab-lufs"
      },
      {
        "heading": "True Peak",
        "body": "True Peak (dBTP) 根据 ITU-R BS.1770-4 使用 4 倍过采样测量样本间峰值。这与原始样本峰值不同，因为有损编解码器（MP3、AAC、Ogg Vorbis、Opus）在编码期间会在样本之间引入过冲，即使底层样本从未达到 0 dBFS 也会造成可听的削波。点击 True Peak 胶囊可查看您测得的 true peak 与每个平台上限的对比。与 LUFS（您希望匹配目标）不同，true peak 是一个上限。保持在目标或以下总是可以的。每行显示平台、其上限（大多数为 -1 dBTP；Spotify Loud 模式和 Amazon Music 为 -2 dBTP），以及「X.X dB 余量」（绿色，您低于上限）或「+X.X dB 超过」（橙色或红色，您高于上限）。主行上 dBTP 读数旁的小徽章遵循相同的颜色规则：低于 -1 dBTP 为绿色，-1 到 0 之间为橙色，高于 0 为红色（会削波 DSP 链的样本间过冲）。将鼠标悬停在平台名称上可查看选择该特定上限的原因。",
        "mockup": "track-tab-truepeak"
      },
      {
        "heading": "质量检查",
        "body": "质量检查胶囊是条件性的。仅当工作器在您的上传中检测到值得标记的内容时才会显示。干净的混音不会显示任何胶囊。显示时，单个轻微问题为琥珀色，多个问题或严重问题为红色。目前会标记三种问题类型：削波（大量样本达到满刻度，同时样本峰值处于或非常接近 0 dBFS，这是限制器上限被压住的典型标志）、满刻度样本峰值（最大样本 ≥ -0.1 dBFS，没有为下游 DSP 或有损编码留出余量）和 DC 偏移（非零平均幅度高于 0.002，通常源于增益级或滤波问题）。点击胶囊会展开一个弹出框，显示每个检测到的问题、简短的技术说明和可操作的修复建议，例如削波：「降低输出增益或检查限制器上限」，DC 偏移：「应用 20 Hz 或更低的高通滤波器」。",
  ...
  {
    "id": "studio-workspace",
    "title": "Studio 标签页：团队、品牌与域名",
    "category": "studio",
    "summary": "在 Studio 标签页中管理您的团队、门户品牌、品牌化客户邮件和自定义门户域名。",
    "tags": [
      "studio",
      "团队",
      "工作区",
    ...
    "content": [
      {
        "heading": "Studio 标签页中有什么",
        "body": "侧边栏中的 [Studio 标签页](/app/studio)将 Studio 方案的工作区功能集中在一处：团队成员、门户品牌、品牌邮件和自定义门户域名。相同的卡片也会出现在[设置](/app/settings)中，因此您可以在任一位置进行管理。在 Free 和 Pro 方案中，大多数卡片会展示该功能的作用并附带升级提示；门户品牌是例外，因为 Pro 方案同样包含此功能。"
      },
      {
        "heading": "团队成员与共享项目",
        "body": "通过电子邮件邀请队友加入您的工作区，并为每个人分配一个角色。工作区中的所有成员共享整个曲库：任何成员都可以打开并处理工作室组内的任何发行，无需按发行逐个邀请。角色决定了他们能做什么。管理员和工程师可以编辑发行、上传音频和管理简报。查看者只有只读权限。只有工作区所有者和管理员可以删除发行。当受邀队友使用受邀的电子邮件地址登录时，邀请会被自动认领，共享的发行会出现在他们的仪表板上。",
        "tip": "对于需要跟进进度但不应更改任何内容的客户或利益相关者，请使用查看者角色。"
      },
      {
        "heading": "门户品牌",
        "body": "上传您的徽标并选择一个强调色，您分享的每个客户门户都会带有您工作室的标识。您可以添加一个可选的深色模式徽标，当客户在深色模式下查看门户时会显示它。SVG 上传（或 2-3 倍尺寸的 PNG 导出）在高分辨率屏幕上保持清晰。在 Studio 方案中，门户是完全白标的：\"Powered by Mix Architect\"页脚会被移除。门户品牌在 Pro 和 Studio 方案中均可使用。"
      },
      {
        "heading": "品牌化客户邮件",
        "body": "在 Studio 方案中，发送给客户的邮件（门户邀请、报价和更新）会以您的工作室名称发出，并带有您的徽标和强调色。设置一个回复地址（Reply-To），让客户的回复直接进入您的收件箱；如果留空，回复将发送到您的账户邮箱。"
      },
      {
        "heading": "自定义门户域名",
        "body": "使用您自己的域名（例如 portal.yourstudio.com）而不是 mixarchitect.com 来提供客户门户。在自定义门户域名卡片中添加域名，然后按照显示的 DNS 记录将其指向 Vercel（子域名使用 CNAME，根域名使用 A 记录，必要时再加一条验证记录）。在记录生效之前，卡片会显示\"等待 DNS\"，之后会变为\"已验证\"，您的门户会自动在新域名上上线，并附带 SSL。",
        "tip": "像 portal.yourstudio.com 这样的子域名是最简单的设置：只需一条 CNAME 记录即可完成。"
      }
    ]
  }
];
