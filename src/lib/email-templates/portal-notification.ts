...
<tr><td style="padding:32px 40px">
  <div style="font-size:14px;font-weight:600;color:#A8841E;margin-bottom:24px">MIX ARCHITECT</div>
  ${content}
  <div style="margin-top:32px;padding-top:16px;border-top:1px solid #eee;font-size:11px;color:#999">
    Sent by <a href="https://mixarchitect.com" style="color:#A8841E;text-decoration:none">Mix Architect</a>
  </div>
</td></tr>
</table>
</td></tr>
...
export function buildNewVersionEmail({ releaseTitle, trackTitle, portalUrl }: NewVersionParams) {
  return {
    ...
    html: wrap(`
      ...
      <a href="${escapeHtml(portalUrl)}" style="display:inline-block;padding:10px 24px;background:#A8841E;color:#1a1a1a;font-size:14px;font-weight:600;text-decoration:none;border-radius:6px">
        Review on Portal
      </a>
    `),
  };
...
export function buildChangesRequestedEmail({ releaseTitle, trackTitle, actorName, note, portalUrl }: ChangesParams) {
  return {
    ...
    html: wrap(`
      ...
      <div style="margin:0 0 16px;padding:12px 16px;background:#f9f5f0;border-left:3px solid #A8841E;border-radius:4px;font-size:14px;color:#333;line-height:1.5">
        ${escapeHtml(note)}
      </div>
      <a href="${escapeHtml(portalUrl)}" style="display:inline-block;padding:10px 24px;background:#A8841E;color:#1a1a1a;font-size:14px;font-weight:600;text-decoration:none;border-radius:6px">
        View Portal
