...
export type DateRangeVariant = "admin" | "app";
...
const accentStyles: Record<
  DateRangeVariant,
  { inRange: string; selected: string; todayDot: string }
> = {
  admin: {
    inRange: "bg-amber-600/8 text-text",
    selected: "bg-amber-600/20 text-amber-500 font-medium",
    todayDot: "bg-amber-500/50",
  },
  app: {
    inRange: "bg-signal-muted text-text",
    selected: "bg-signal-muted text-signal font-medium",
    todayDot: "bg-signal/50",
  },
};
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  inRange && !isSelected && accent.inRange,
              ...
              >
                {date.getDate()}
                {isToday && (
                  <span className={cn("absolute bottom-0.5 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full", accent.todayDot)} />
                )}
              </button>
            );
          })}
