        ...
        <>
          <div className="flex items-center gap-2 text-sm">
            ...
            {verified ? (
              ...
              <span className="text-xs font-medium uppercase tracking-wide text-amber-500">{t("pendingDns")}</span>
            )}
            <button
              type="button"
          ...
          {verified ? (
            ...
            <>
              <div className="space-y-2">
                <p className="text-xs text-muted">{t("pointDomain")}</p>
                <div className="rounded-md border border-border p-3 font-mono text-[11px] text-text space-y-1" style={{ background: "var(--panel-2)" }}>
                  <div>{t("dnsSubdomainLabel")} → CNAME → <span className="text-signal">cname.vercel-dns.com</span></div>
                  <div>{t("dnsApexLabel")} → A → <span className="text-signal">76.76.21.21</span></div>
                </div>
                {row.verification && row.verification.length > 0 && (
                  <div className="rounded-md border border-border p-3 font-mono text-[11px] text-text space-y-1" style={{ background: "var(--panel-2)" }}>
                    <div className="text-faint mb-1">{t("verificationRecord")}</div>
